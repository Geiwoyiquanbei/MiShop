...
func GoodsTypeAttrController(c *gin.Context) {
	cateId, err1 := strconv.Atoi(c.Query("cateId"))
	goodsTypeAttributeList := []models.GoodsTypeAttribute{}
	err2 := mysql.DB.Where("cate_id = ?", cateId).Find(&goodsTypeAttributeList).Error
...
func GoodsDoAddController(c *gin.Context) {
	//1、获取表单提交过来的数据 进行判断
	...
	//注意小数点
	marketPrice, _ := logic.Float(c.PostForm("market_price"))
	price, _ := logic.Float(c.PostForm("price"))
	...
	//3、上传图片   生成缩略图
	goodsImg, _ := logic.UpLoadImg(c, "goods_img")
	...
	if err != nil {
		logic.ErrorReply(c, "增加失败", "/admin/goods/add")
	}
	//5、增加图库 信息
	wg.Add(1)
	go func() {
		...
		for _, v := range goodsImageList {
			...
			goodsImgObj.AddTime = int(logic.GetUnix())
	...
	go func() {
		...
		for i := 0; i < len(attrIdList); i++ {
			...
			if attributeIdErr == nil {
				...
				goodsAttrObj.AddTime = int(logic.GetUnix())
...
func GoodsEditController(c *gin.Context) {
	...
	for _, v := range goodsAttr {
		if v.AttributeType == 1 {
			...
			mysql.DB.Find(&goodsTypeArttribute)
...
func GoodsDoEditController(c *gin.Context) {
	...
	//4、上传图片   生成缩略图
	goodsImg, err2 := logic.UpLoadImg(c, "goods_img")
	...
	if err3 != nil {
		logic.ErrorReply(c, "修改失败", "/admin/goods/edit?id="+strconv.Itoa(id))
		return
	}
...
func ChangeGoodsImageColor(c *gin.Context) {
	...
	mysql.DB.Find(&goodsImage)
	goodsImage.ColorId = colorId
	err3 := mysql.DB.Save(&goodsImage).Error
...
func RemoveGoodsImage(c *gin.Context) {
	...
	goodsImageId, err1 := strconv.Atoi(c.Query("goods_image_id"))
	goodsImage := models.GoodsImage{Id: goodsImageId}
	err2 := mysql.DB.Delete(&goodsImage).Error
	if err1 != nil || err2 != nil {
		c.JSON(http.StatusOK, gin.H{
			"result":  "删除失败",
			"success": false,
